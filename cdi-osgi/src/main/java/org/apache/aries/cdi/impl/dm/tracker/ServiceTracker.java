...
public class ServiceTracker<S, T> implements ServiceTrackerCustomizer<S, T> {
    ...
    public ServiceTracker(final BundleContext context, final Filter filter,
        ...
        this.trackReference = null;
        this.trackClass = null;
        this.listenerFilter = filter != null ? filter.toString() : null;
        this.filter = filter;
        this.customizer = (customizer == null) ? this : customizer;
                    ...
                    if (trackClass != null) {
                        ...
                        if (trackReference != null) {
                            ...
                            references = getInitialReferences(trackAllServices, null,
                                    (listenerFilter != null) ? listenerFilter : filter.toString());
                        }
                    }
    ...
    public void close() {
        ...
        if (references != null) {
            for (ServiceReference<S> reference : references) {
                outgoing.untrack(reference, null).execute();
            }
        }
        if (DEBUG) {
    ...
    public ServiceReference<S>[] getServiceReferences() {
        ...
        synchronized (t) {
            ...
            return t.getTracked(new ServiceReference[length]);
        }
    }
     ...
     * @ThreadSafe
     */
    class Tracked extends AbstractTracked<ServiceReference<S>, ServiceEvent, T> implements ServiceListener {
        /**
         * Tracked constructor.
         */
        Tracked() {
        ...
        public void serviceChanged(final ServiceEvent event) {
            ...
            @SuppressWarnings("unchecked")
            final ServiceReference<S> reference = (ServiceReference) event.getServiceReference();
            if (debug) {
         ...
         */
        T customizerAdding(final ServiceReference<S> item, final ServiceEvent related) {
            return customizer.addingService(item);
        }

        void customizerAdded(final ServiceReference<S> item, final ServiceEvent related, final T object) {
         ...
         */
        void customizerModified(final ServiceReference<S> item, final ServiceEvent related, final T object) {
         ...
         * @param object  Customized object for the tracked item.
         */
        void customizerRemoved(final ServiceReference<S> item, final ServiceEvent related, final T object) {
            customizer.removedService(item, object);
        }

        @Override
        AbstractCustomizerActionSet<ServiceReference<S>, ServiceEvent, T> createCustomizerActionSet() {
            ...
            return new AbstractCustomizerActionSet<ServiceReference<S>, ServiceEvent, T>() {
                @Override
                void execute() {
                    // inspect the actions and check whether we should perform a swap
                    List<CustomizerAction<ServiceReference<S>, ServiceEvent, T>> actions = getActions();
                    if (actions.size() > 2) {
                        throw new IllegalStateException("Unexpected action count: " + actions.size());
                    }
                    // just sequentially call the customizer methods
                    for (CustomizerAction<ServiceReference<S>, ServiceEvent, T> action : getActions()) {
                        try {
